...
io.on("connection", (socket) => {
  ...
  socket.on("newProduct", async (newProduct) => {
    try {
      await axios.post(
        "http://localhost:3000/api/manager/addProduct",
        newProduct
      );
  ...
  socket.on("updateProduct", async (id, updatedProduct) => {
    try {
      await axios.put("http://localhost:3000/api/manager/update", {
        id,
        updatedProduct,
      });
      socket.emit("updateTable");
  ...
  socket.on("cart", async (list) => {
    try {
      await axios.post("http://localhost:3000/api/carts/addCart", {
        products: list,
      });
    } catch (error) {
      console.error(error);
