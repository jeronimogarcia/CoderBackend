...
const userRoutes = (io) => {
  ...
  router.get("/users/:id?", validate, async (req, res) => {
    try {
      ...
      res
        .status(500)
        .send({ status: "ERR", error: "No se encuentra el usuario" });
    }
  });
  ...
  router.post("/users", validate, async (req, res) => {
    try {
      ...
      res
        .status(500)
        .send({ status: "ERR", error: "No se puede agregar el usuario" });
    }
  });
  ...
  router.put("/users/:id", validate, async (req, res) => {
    try {
      ...
      res
        .status(500)
        .send({ status: "ERR", error: "No se puede actualizar el usuario" });
    }
  });
